...
AsyncServer::AsyncServer(EventLoop* loop, const InetAddress& addr,
    ...
    // connectionCallback fires when TcpConnection is fully established.
    // We eagerly wrap it in AsyncConnection RIGHT HERE so message/write
    // callbacks are bound before any data can arrive on the socket.
...
void AsyncServer::onConnection(const TcpConnectionPtr& conn) {
    ...
    // Build AsyncConnection here (outside the lock) so its ctor can
    // call setMessageCallback / setWriteCompleteCallback immediately.
    // Any data arriving after this point will be buffered safely.
